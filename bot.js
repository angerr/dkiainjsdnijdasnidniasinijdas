...
client.on("message", message => { // تقديم اداره
  if(message.content.startsWith("ANapply")) {
  ...
  if(message.content.startsWith("ANقبول")) {
...
client.on('message',async message => {
  let mention = message.mentions.members.first();
  if(message.content.startsWith("ANرفض")) {
  if(!message.channel.guild) return;
  let acRoom = message.guild.channels.find('name', '💥-accept-or-reject');
  if(!message.guild.member(message.author).hasPermission("ADMINISTRATOR")) return;
  if(!mention) return message.reply("منشن شخص");
...
client.on("message", message => {
    if (message.content === (prefix + "ANhelp")) {
     const embed = new Discord.RichEmbed() 
         .setColor("#580e6b")
         .setThumbnail(message.author.avatarURL)
         .setDescription(`**
         bot by : Wicked
         tech server : AnotherLife
       **  `)
...
client.on('message', message => {
  ...
  if (message.content.startsWith(adminprefix + 'sets')) {
    client.user.setGame(argresult, "https://www.youtube.com/channel/UCN98b3DPXiLUeFEwe-O-ntw?view_as=subscriber");
